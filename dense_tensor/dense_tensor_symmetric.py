...
class DenseTensorSymmetric(Layer):
    ...
    def __init__(self, output_dim, q=10, alpha=1e-3, beta=1, init='glorot_uniform', activation='linear', weights=None,
    ...
    def build(self, input_shape):
        ...
        self.alpha = K.variable(np.float32(self.initial_alpha), name="{}_alpha".format(self.name))
        self.beta = K.variable(np.float32(self.initial_beta), name="{}_beta".format(self.name))

        self.W = self.init((input_dim, self.output_dim),
                           name='{}_W'.format(self.name))  # m,p
        self.Q = self.init((self.output_dim, input_dim, self.q),
                           name='{}_Q'.format(self.name))  # p,m,q
        tmp = T.batched_tensordot(self.Q, self.Q, axes=[[2], [2]])  # p,m,q + p,m,q = p,m,m
        self.V = self.beta * (T.eye(input_dim, input_dim, k=0).dimshuffle(['x', 0, 1]) * self.alpha + tmp)  # m,p,p

        if self.bias:
            self.b = K.zeros((self.output_dim,),
                             name='{}_b'.format(self.name))
            self.trainable_weights = [self.W, self.Q, self.b]
        else:
            self.trainable_weights = [self.W, self.Q]
